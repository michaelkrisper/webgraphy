...
- `npm run e2e` — Playwright smoke + visual regression against the production preview build (needs `npx playwright install chromium`). Screenshot baselines live in `e2e/**-snapshots/` and are generated by CI in the pinned Playwright container, never locally — GPU rasterisation differs per machine.
