...
function optimized() {
  ...
  // Create mapping array of just total sizes
  const leftTotals = new Float64Array(leftAxes.length);
  for (let i = 0; i < leftAxes.length; i++) leftTotals[i] = axisLayout[leftAxes[i].id]?.total || 40;

  const rightTotals = new Float64Array(rightAxes.length);
  for (let i = 0; i < rightAxes.length; i++) rightTotals[i] = axisLayout[rightAxes[i].id]?.total || 40;
  ...
  yAxes.forEach((axis) => {
    ...
    let xPos = 0;
    if (isLeft) {
      let offset = 0; for (let i = 0; i < sideIdx; i++) offset += leftTotals[i];
      xPos = padding.left - offset - axisMetrics.total;
    } else {
      let offset = 0; for (let i = 0; i < sideIdx; i++) offset += rightTotals[i];
      xPos = width - padding.right + offset;
    }
    count += xPos;
  });
...
console.log(`Optimized (Float64Array): ${(end2 - start2).toFixed(2)}ms`);
