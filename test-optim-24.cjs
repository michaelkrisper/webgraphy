...
function optimizedWithMemoObject() {
  let count = 0;
  yAxes.map((axis) => {
    ...
    let xPos = 0;
    if (isLeft) {
      xPos = padding.left - yAxisOffsets[axis.id] - axisMetrics.total;
    } else {
      xPos = width - padding.right + yAxisOffsets[axis.id];
    }
    count += xPos;
  });
  return count;
...
for(let i=0; i<100000; i++) sum2 += optimizedWithMemoObject();
const end2 = performance.now();

console.log(`Baseline YAxes map: ${(end1 - start1).toFixed(2)}ms`);
console.log(`Optimized map with precomputed memo object: ${(end2 - start2).toFixed(2)}ms`);
