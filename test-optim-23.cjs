...
function optimized() {
  ...
  // To match React, we use useMemo, so we only compute yAxisOffsets once when dependencies change
  // Then the map function just uses it. Let's simulate that by moving the Map creation OUT of the function.
  return count;
}
...
for(let i=0; i<100000; i++) sum2 += optimizedWithMemo();
const end2 = performance.now();

console.log(`Baseline YAxes map: ${(end1 - start1).toFixed(2)}ms`);
console.log(`Optimized map with precomputed memo map: ${(end2 - start2).toFixed(2)}ms`);
