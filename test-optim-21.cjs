...
function optimized() {
  ...
  const leftOffsets = new Float64Array(leftAxes.length);
  ...
  const rightOffsets = new Float64Array(rightAxes.length);
...
console.log(`Optimized YAxes map with manual loop find: ${(end2 - start2).toFixed(2)}ms`);
