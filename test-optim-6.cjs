...
function optimized() {
  ...
  // Use simple tracking while iterating over axes directly if possible,
  // but map uses yAxes instead. Let's precompute a Map of offsets:
  const leftOffsets = new Map();
  let leftCumulative = 0;
  for (let i = 0; i < leftAxes.length; i++) {
    leftOffsets.set(leftAxes[i].id, leftCumulative);
  ...
  for (let i = 0; i < rightAxes.length; i++) {
    rightOffsets.set(rightAxes[i].id, rightCumulative);
...
console.log(`Optimized (Map direct): ${(end2 - start2).toFixed(2)}ms`);
