...
function optimized() {
  ...
  // Use a map to store total offset for each ID to avoid indexOf in map callback
  const offsets = {};
  ...
  yAxes.forEach((axis) => {
    ...
    let xPos = 0;
    if (isLeft) {
      xPos = padding.left - offsets[axis.id] - axisMetrics.total;
    } else {
      xPos = width - padding.right + offsets[axis.id];
    }
    count += xPos;
  });
  return count;
...
console.log(`Optimized (Object Map): ${(end2 - start2).toFixed(2)}ms`);
