...
describe('evaluateFuncToken', () => {
	it('evaluates basic math functions', () => {
		const token = (val: string): Extract<Token, { type: "FUNC" }> => ({ type: "FUNC", value: val, id: 1, pos: 0 });
		...
		expect(evaluateFuncToken(token('atan2'), [1, 1], [], [], 0)).toBe(Math.atan2(1, 1));
	...
	it('returns first argument for unknown function', () => {
		const token = (val: string): Extract<Token, { type: "FUNC" }> => ({ type: "FUNC", value: val, id: 1, pos: 0 });
		expect(evaluateFuncToken(token('unknown_func'), [42, 10], [], [], 0)).toBe(42);
	});
});

describe('evaluateBucketFunc', () => {
	it('returns first argument when context is missing', () => {
		const token: Extract<Token, { type: "FUNC" }> = { type: "FUNC", value: 'avgday', id: 1, pos: 0 };
		expect(evaluateBucketFunc(token, 42, [], 0)).toBe(42);
	});

	it('evaluates bucket functions with context', () => {
		const tokenAvg: Extract<Token, { type: "FUNC" }> = { type: "FUNC", value: 'avgday', id: 1, pos: 0 };
		const tokenSum: Extract<Token, { type: "FUNC" }> = { type: "FUNC", value: 'sumday', id: 2, pos: 0 };
		const ctx = {
			groupSums: {},
			groupCounts: {},
			groupLastKey: {},
...
describe('evaluateRolling', () => {
	it('returns first argument when context is missing', () => {
		const token: Extract<Token, { type: "FUNC" }> = { type: "FUNC", value: 'rolling', id: 1, pos: 0 };
		expect(evaluateRolling(token, [42], [], 0)).toBe(42);
	});

	it('evaluates rolling function', () => {
		const token: Extract<Token, { type: "FUNC" }> = { type: "FUNC", value: 'rolling', id: 1, pos: 0, constN: 2 };
		const ctx = {
			queues: {},
			sums: {},
		} as unknown as FormulaContext;
	...
	it('evaluates rolling time function', () => {
		const token: Extract<Token, { type: "FUNC" }> = { type: "FUNC", value: 'rollingtime', id: 1, pos: 0, constN: 10 };
		const ctx = {
			timeQueues: {},
			timeSums: {},
		} as unknown as FormulaContext;
	...
	it('evaluates other rolling variants', () => {
		...
		expect(evaluateRolling({ type: "FUNC", value: 'rollingmed', id: 2, pos: 0, constN: 3 }, [10], [], 0, ctx)).toBe(10);
		expect(evaluateRolling({ type: "FUNC", value: 'rollingmed', id: 2, pos: 0, constN: 3 }, [20], [], 0, ctx)).toBe(15);
		expect(evaluateRolling({ type: "FUNC", value: 'rollingmed', id: 2, pos: 0, constN: 3 }, [30], [], 0, ctx)).toBe(20);

		expect(evaluateRolling({ type: "FUNC", value: 'rollingmin', id: 3, pos: 0, constN: 3 }, [10], [], 0, ctx)).toBe(10);
		expect(evaluateRolling({ type: "FUNC", value: 'rollingmin', id: 3, pos: 0, constN: 3 }, [5], [], 0, ctx)).toBe(5);

		expect(evaluateRolling({ type: "FUNC", value: 'rollingmax', id: 4, pos: 0, constN: 3 }, [10], [], 0, ctx)).toBe(10);
		expect(evaluateRolling({ type: "FUNC", value: 'rollingmax', id: 4, pos: 0, constN: 3 }, [20], [], 0, ctx)).toBe(20);

		expect(evaluateRolling({ type: "FUNC", value: 'rollingstd', id: 5, pos: 0, constN: 3 }, [10], [], 0, ctx)).toBe(0);
		expect(evaluateRolling({ type: "FUNC", value: 'rollingstd', id: 5, pos: 0, constN: 3 }, [20], [], 0, ctx)).toBeCloseTo(7.071);

		// unknown rolling
		expect(evaluateRolling({ type: "FUNC", value: 'rolling_unknown', id: 6, pos: 0 }, [42], [], 0, ctx)).toBe(42);
	});
});

describe('evaluateRowRelative', () => {
	it('returns first argument when context is missing', () => {
		const token: Extract<Token, { type: "FUNC" }> = { type: "FUNC", value: 'lag', id: 1, pos: 0 };
		expect(evaluateRowRelative(token, [42])).toBe(42);
	});

	it('evaluates lag', () => {
		const token: Extract<Token, { type: "FUNC" }> = { type: "FUNC", value: 'lag', id: 1, pos: 0, constN: 1 };
	...
	it('evaluates diff', () => {
		const token: Extract<Token, { type: "FUNC" }> = { type: "FUNC", value: 'diff', id: 2, pos: 0 };
	...
	it('evaluates cumsum, cumprod, cummax, cummin', () => {
		...
		expect(evaluateRowRelative({ type: "FUNC", value: 'cumsum', id: 3, pos: 0 }, [10], ctx)).toBe(10);
		expect(evaluateRowRelative({ type: "FUNC", value: 'cumsum', id: 3, pos: 0 }, [20], ctx)).toBe(30);

		expect(evaluateRowRelative({ type: "FUNC", value: 'cumprod', id: 4, pos: 0 }, [2], ctx)).toBe(2);
		expect(evaluateRowRelative({ type: "FUNC", value: 'cumprod', id: 4, pos: 0 }, [3], ctx)).toBe(6);

		expect(evaluateRowRelative({ type: "FUNC", value: 'cummax', id: 5, pos: 0 }, [10], ctx)).toBe(10);
		expect(evaluateRowRelative({ type: "FUNC", value: 'cummax', id: 5, pos: 0 }, [5], ctx)).toBe(10);
		expect(evaluateRowRelative({ type: "FUNC", value: 'cummax', id: 5, pos: 0 }, [20], ctx)).toBe(20);

		expect(evaluateRowRelative({ type: "FUNC", value: 'cummin', id: 6, pos: 0 }, [10], ctx)).toBe(10);
		expect(evaluateRowRelative({ type: "FUNC", value: 'cummin', id: 6, pos: 0 }, [15], ctx)).toBe(10);
		expect(evaluateRowRelative({ type: "FUNC", value: 'cummin', id: 6, pos: 0 }, [5], ctx)).toBe(5);

		// unknown func
		expect(evaluateRowRelative({ type: "FUNC", value: 'unknown_cum', id: 7, pos: 0 }, [42], ctx)).toBe(42);
	});
});
...
describe('evaluateFuncToken stateful variants', () => {
	...
	it('delegates bucket functions', () => {
		const token = (val: string): Extract<Token, { type: "FUNC" }> => ({ type: "FUNC", value: val, id: 1, pos: 0 });
		const ctx = { groupSums: {}, groupCounts: {}, groupLastKey: {} } as unknown as FormulaContext;
		expect(evaluateFuncToken(token('avgday'), [10], [0, 1600000000000], [], 1, ctx)).toBe(10);
	});

	it('delegates rolling functions', () => {
		const token = (val: string): Extract<Token, { type: "FUNC" }> => ({ type: "FUNC", value: val, id: 1, pos: 0, constN: 2 });
	...
	it('delegates row relative functions', () => {
		const token = (val: string): Extract<Token, { type: "FUNC" }> => ({ type: "FUNC", value: val, id: 1, pos: 0, constN: 1 });
		const ctx = { lagBuffers: {} } as unknown as FormulaContext;
...
describe('evaluateFuncToken fallback logic', () => {
    it('returns a if not matched by any block', () => {
		const token = (val: string): Extract<Token, { type: "FUNC" }> => ({ type: "FUNC", value: val, id: 1, pos: 0 });
		expect(evaluateFuncToken(token('unknown_entirely'), [99], [], [], 0)).toBe(99);
    });
});
