...
function processCSVRow(
  ...
  if (delimLen === 1) {
    ...
    for (let k = 0; k < numActive; k++) {
      ...
      data[k][actualRowCount] = parsedNum;
    }
  } else {
...
function parseValueFast(
  ...
  if (config?.type === "date" || config?.type === "categorical") {
    const val = line.substring(vStart, vEnd + 1);
  ...
  while (i <= vEnd) {
    ...
    } else if ((!isComma && char === 46) || (isComma && char === 44)) {
      // '.' or ','
      if (isDecimal) {
        isClean = false;
        break; // too many decimals
      }
      isDecimal = true;
    } else {
      isClean = false;
      break; // invalid character, exponential, etc.
    }
    i++;
  }

  // Fallback if not a clean number or no digits found
  if (!isClean || !hasDigit || i <= vEnd) {
    let fallbackStr = "";
    for (let j = vStart; j <= vEnd; j++) {
      const c = line.charCodeAt(j);
      if (isComma && c === 44) fallbackStr += ".";
      else fallbackStr += String.fromCharCode(c);
    }
    const p = parseFloat(fallbackStr);
    return Number.isNaN(p) ? NaN : p;
  }

  // Quick fast-path for valid clean numbers without falling back to JS string creation
  if (decimalPlaces > 0) {
