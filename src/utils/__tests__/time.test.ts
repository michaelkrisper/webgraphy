...
describe("generateTimeTicks", () => {
	it("generates ticks for multi-month steps", () => {
		const ticks = generateTimeTicks(0, 86400 * 180, {
			unit: "month",
			value: 2,
		});
	...
	it("generates ticks for multi-year steps", () => {
		const ticks = generateTimeTicks(0, 86400 * 365 * 5, {
			unit: "year",
			value: 2,
		});
		expect(ticks.length).toBeGreaterThan(0);
	...
	it("generates ticks for weeks", () => {
		...
		expect(ticks.map((t) => t.timestamp)).toEqual([
			-864000, -259200, 345600, 950400, 1555200,
		]);
		expect(ticks.map((t) => t.label)).toEqual([
			"22.12.",
			"29.12.",
	...
	it("generates ticks for months", () => {
		...
		expect(ticks.map((t) => t.timestamp)).toEqual([
			-2678400, 0, 2678400, 5097600, 7776000,
		]);
	...
	it("generates ticks for years", () => {
		...
		const ticks = generateTimeTicks(0, 86400 * 365 * 2, {
			unit: "year",
			value: 1,
		});
		expect(ticks.map((t) => t.timestamp)).toEqual([
			-31536000, 0, 31536000, 63072000, 94694400,
		]);
		expect(ticks.map((t) => t.label)).toEqual([
			"1969",
			"1970",
	...
	it("returns empty label for unknown unit", () => {
		const ticks = generateTimeTicks(0, 10, {
...
describe("getTimeStep", () => {
	...
	it("returns a suitable step in years for a very long range", () => {
		...
		expect(getTimeStep(10 * 365 * 86400, 5)).toEqual({
			unit: "year",
			value: 2,
		});
	});
	...
	it("returns the largest available step if the ideal step exceeds all options", () => {
		...
		expect(getTimeStep(1000 * 365 * 86400, 5)).toEqual({
			unit: "year",
			value: 100,
		});
	});
