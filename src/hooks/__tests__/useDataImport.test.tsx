...
describe("useDataImport hook", () => {
	...
	it("should process import with parseData successfully", async () => {
		...
		vi
			.mocked(parseDataInWorker)
			.mockResolvedValueOnce([mockDataset] as unknown as ReturnType<
				typeof parseDataInWorker
			>);
	...
	it("should handle error when reading an Excel file fails", async () => {
		...
		vi
			.mocked(readExcelFileInWorker)
			.mockRejectedValueOnce(new Error("Failed to parse Excel file."));
	...
	it.each([
		{
			label: "an Error",
			thrown: new Error("changeSheet failed"),
			expected: "changeSheet failed",
		},
		{
			label: "a string",
			thrown: "String error in changeSheet",
			expected: "String error in changeSheet",
		},
	])(
		"surfaces $label thrown by changeSheetInWorker during changeSheet",
	...
	it.each([
		{
			label: "an Error",
			thrown: new Error("addDataset failed"),
			expected: "addDataset failed",
		},
		{
			label: "a string",
			thrown: "String error in confirmImport",
			expected: "String error in confirmImport",
		},
	])(
		"surfaces $label thrown by addDataset during confirmImport",
		async ({ thrown, expected }) => {
			const { result } = renderHook(() => useDataImport());
	...
	it.each([
		{
			label: "an Error",
			thrown: new Error("File read failed"),
			expected: "File read failed",
		},
		{ label: "a string", thrown: "String error", expected: "String error" },
	])(
		"surfaces $label thrown synchronously during initiateImport",
		async ({ thrown, expected }) => {
			const { result } = renderHook(() => useDataImport());
	...
	it.each([
		{
			label: "an Error",
			thrown: new Error("parseDataInWorker failed"),
			expected: "parseDataInWorker failed",
		},
		{
			label: "a string",
			thrown: "String error in parseDataInWorker",
			expected: "String error in parseDataInWorker",
		},
	])(
		"surfaces $label thrown by parseDataInWorker during confirmImport",
		async ({ thrown, expected }) => {
			const { result } = renderHook(() => useDataImport());
			const file = new File(["dummy"], "test.csv", { type: "text/csv" });
