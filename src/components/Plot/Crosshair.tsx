  ...
  ({
    ...
    const computeSnap = useCallback(
      (pos: { x: number; y: number }): SnapResult | null => {
        ...
        const closestIndices = new Int32Array(seriesMetadata.length);
        const seenDatasets = new Set<string>();

        for (let i = 0; i < seriesMetadata.length; i++) {
          const { ds, xAxis, xCol } = seriesMetadata[i];
          let cachedIdx = -1;

          if (seenDatasets.has(ds.id)) {
            for (let j = 0; j < i; j++) {
              if (seriesMetadata[j].ds.id === ds.id) {
                cachedIdx = closestIndices[j];
                break;
              }
            }
          } else {
          ...
          const sMouseWorld = screenToWorld(pos.x, pos.y, sVp);
          for (let idxOffset = -1; idxOffset <= 1; idxOffset++) {
            const idx = cachedIdx + idxOffset;
            if (idx < 0 || idx >= xData.length) continue;
            const wx = xData[idx] + refX;
        ...
        for (let i = 0; i < seriesMetadata.length; i++) {
          const { series: s, ds, xAxis, xCol, yCol, axis } = seriesMetadata[i];
          const xData = xCol.data,
            yData = yCol.data;
          const refX = xCol.refPoint,
            refY = yCol.refPoint;
          const bestI = closestIndices[i];
