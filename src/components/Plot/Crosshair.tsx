  ...
  ({
    ...
    const updateCrosshair = useCallback(
      (pos: { x: number; y: number } | null) => {
        ...
        drawCanvas(snap, pos);
        setSnapState({ snap, pos });
      },
      [computeSnap, drawCanvas],
    );
      ...
      <>
        ...
        <div
          ...
          style={{
            ...
            display:
              !snapState.snap || !snapState.pos || isPanning ? "none" : "",
            left: snapState.pos ? `${snapState.pos.x + 12}px` : undefined,
            top: snapState.pos ? `${snapState.pos.y + 15}px` : undefined,
          }}
        >
          {snapState.snap &&
            snapState.pos &&
            !isPanning &&
            snapState.snap.entries.map((group, gIdx) => (
              <div
                key={gIdx}
                style={{
                  color: tooltipSubColor,
                  fontSize: "9px",
                  borderTop: gIdx > 0 ? `1px solid ${tooltipDividerColor}` : "",
                  paddingTop: gIdx > 0 ? "4px" : "",
                  marginTop: gIdx > 0 ? "4px" : "",
                }}
              >
                <span
                  className="chart-tooltip-x-label"
                  style={{ color: tooltipColor }}
                >
                  {(snapState.snap!.entries.length > 1
                    ? `${group.xAxisName}: `
                    : "") + group.xLabel}
                </span>
                <div className="chart-tooltip-items">
                  {group.items.map((item, iIdx) => {
                    ...
                    const decPart =
                      sepIdx === -1 ? "" : formatted.slice(sepIdx);
                    return (
                      <div
                        key={iIdx}
                        className="chart-tooltip-item-row"
                      ...
                      >
                        ...
                        <span
                          className="chart-tooltip-value-int"
                          style={{ color: tooltipColor }}
                        >
                          {intPart}
                        </span>
                        <span
                          className="chart-tooltip-value-dec"
                          style={{ color: tooltipColor }}
                        >
                          {decPart}
                        </span>
