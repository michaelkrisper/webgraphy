...
const AxesLayer = React.memo(forwardRef<AxesLayerHandle, AxesLayerProps>(({ 
  xAxes, yAxes, width, height, padding, series, axisLayout, 
  xAxesMetrics, axisColor, zeroLineColor, labelColor, 
  secLabelBg, leftOffsets, rightOffsets 
}: AxesLayerProps, ref) => {
  ...
  useEffect(() => {
    const drawFrame = (currentXAxes: XAxisLayout[], currentYAxes: YAxisLayout[]) => {
      ...
      // Y Axes Zero line
      const mainXAxis = currentXAxes[0];
      if (currentYAxes.length > 0 && mainXAxis) {
        const mainAxis = currentYAxes[0];
        const axisVp = { xMin: mainXAxis.min, xMax: mainXAxis.max, yMin: mainAxis.min, yMax: mainAxis.max, width, height, padding };
        if (mainAxis.min <= 0 && mainAxis.max >= 0) {
          const { y } = worldToScreen(mainXAxis.min, 0, axisVp);
          ctx.strokeStyle = zeroLineColor;
          ctx.setLineDash([4, 4]);
          ctx.beginPath();
          ctx.moveTo(padding.left, Math.round(y) + 0.5);
      ...
      currentYAxes.forEach((axis) => {
        ...
        axis.ticks.forEach(t => {
          const { y } = worldToScreen(mainXAxis.min, t, { xMin: mainXAxis.min, xMax: mainXAxis.max, yMin: axis.min, yMax: axis.max, width, height, padding });
  ...
  }, [xAxes, yAxes, width, height, padding, seriesByYAxisId, axisLayout, xAxesMetrics, axisColor, zeroLineColor, labelColor, secLabelBg, leftOffsets, rightOffsets, isMobile]);
