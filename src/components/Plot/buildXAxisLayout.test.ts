...
describe("buildXAxisLayout", () => {
	...
	const defaultDatasets = [{ xAxisColumn: "time" } as Dataset];

	it("handles zero range or zero width", () => {
		const result1 = buildXAxisLayout(
			{ ...defaultAxis, min: 100, max: 100 },
			500,
			"red",
			undefined,
			undefined,
		...
		const result2 = buildXAxisLayout(
			defaultAxis,
			0,
			"red",
			undefined,
