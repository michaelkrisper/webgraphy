...
import { FilePlus, Trash2, ChevronRight, ChevronDown, HelpCircle, X, Eye, FileImage, Image, Calculator, ArrowUpDown, Hash, MoveHorizontal, Rows, SquareSplitVertical, Square, Minus, Circle, Palette, Sun, Moon, Terminal, Sparkles, List, FlaskConical, RotateCcw, Save, FolderOpen, Clock } from 'lucide-react';
      ...
      <aside className="sidebar" style={{ width, position: 'relative', display: 'flex', flexDirection: 'column', height: '100%', backgroundColor: 'var(--bg2)', borderLeft: '1px solid var(--border-color)', boxShadow: '-2px 0 10px var(--shadow)', flexShrink: 0, zIndex: 1000 }}>
        ...
        <header className="sb-header">
          ...
          <div className="sb-hdr-btns">
            ...
            {hdrBtn(() => { const ax = xAxes[0]; if (ax) updateXAxis(ax.id, { showGrid: !ax.showGrid }); }, xAxes[0]?.showGrid ? <SquareSplitVertical size={16} /> : <Square size={16} />, xAxes[0]?.showGrid ? 'Hide Vertical Grid' : 'Show Vertical Grid', xAxes[0]?.showGrid ? 'var(--accent)' : undefined)}
